...
export default class ContextFilterTestsTools {
    ...
    private initializeAnonymizationFieldConfVO() {
        ...
        let datatable_fields = [
            ...
            new ModuleTableField(field_names<AnonymizationFieldConfVO>().field_id, ModuleTableField.FIELD_TYPE_string, 'FIELD ID', true),
    ...
    private initializeAnonymizationUserConfVO() {
        ...
        let anon_field_id = new ModuleTableField(field_names<AnonymizationUserConfVO>().anon_field_id, ModuleTableField.FIELD_TYPE_foreign_key, 'Champs anonymisé', true);
        let datatable_fields = [
            user_id,
            anon_field_id
        ];
    ...
    private declare_TranslatableTextVO_modultable() {
        let label_field = new ModuleTableField(field_names<TranslatableTextVO>().code_text, ModuleTableField.FIELD_TYPE_string, 'Id du text', true).unique();
        let datatable_fields = [
            label_field
        ];
    ...
    private declare_TranslationVO_modultable() {
        ...
        let label_field = new ModuleTableField(field_names<TranslationVO>().translated, ModuleTableField.FIELD_TYPE_string, 'Texte traduit', true);
        let datatable_fields = [
            field_lang_id,
            field_text_id,
            label_field
    ...
    private declare_UserVO_modultable() {
        ...
        let field_lang_id = new ModuleTableField(field_names<UserVO>().lang_id, ModuleTableField.FIELD_TYPE_foreign_key, new DefaultTranslation({ 'fr-fr': 'Langue' }), true);
        let label_field = new ModuleTableField(field_names<UserVO>().name, ModuleTableField.FIELD_TYPE_string, new DefaultTranslation({ 'fr-fr': 'Nom' }), true);
        let datatable_fields = [
            label_field,
            ...
            new ModuleTableField(field_names<UserVO>().email, ModuleTableField.FIELD_TYPE_email, new DefaultTranslation({ 'fr-fr': 'E-mail' }), true),
            new ModuleTableField(field_names<UserVO>().phone, ModuleTableField.FIELD_TYPE_string, new DefaultTranslation({ 'fr-fr': 'Téléphone' })),
            new ModuleTableField(field_names<UserVO>().blocked, ModuleTableField.FIELD_TYPE_boolean, new DefaultTranslation({ 'fr-fr': 'Compte bloqué' }), true, true, false),
            ...
            new ModuleTableField(field_names<UserVO>().password_change_date, ModuleTableField.FIELD_TYPE_tstz, new DefaultTranslation({ 'fr-fr': 'Date de changement du mot de passe' }), false),
