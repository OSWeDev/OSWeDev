...
export default class PostAPIDefinition<T, U> extends APIDefinition<T, U> {
    ...
    ): PostAPIDefinition<T, U> {
        return new PostAPIDefinition<T, U>(access_policy_name, APIControllerWrapper.get_api_name_from_module_function(module_name, function_name), API_TYPES_IDS_involved, param_translator, api_return_type);
    }
}