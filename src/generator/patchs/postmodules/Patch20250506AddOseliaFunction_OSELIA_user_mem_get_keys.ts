...
export default class Patch20250506AddOseliaFunction_OSELIA_user_mem_get_keys implements IGeneratorWorker {

    private static instance: Patch20250506AddOseliaFunction_OSELIA_user_mem_get_keys = null;

    private constructor() { }

    get uid(): string {
        return 'Patch20250506AddOseliaFunction_OSELIA_user_mem_get_keys';
    }

    // istanbul ignore next: nothing to test
    public static getInstance(): Patch20250506AddOseliaFunction_OSELIA_user_mem_get_keys {
        if (!Patch20250506AddOseliaFunction_OSELIA_user_mem_get_keys.instance) {
            Patch20250506AddOseliaFunction_OSELIA_user_mem_get_keys.instance = new Patch20250506AddOseliaFunction_OSELIA_user_mem_get_keys();
        }
        return Patch20250506AddOseliaFunction_OSELIA_user_mem_get_keys.instance;
    }


    public async work(db: IDatabase<any>) {

        let user_mem_get_keys_function = await query(GPTAssistantAPIFunctionVO.API_TYPE_ID)
            .filter_by_text_eq(field_names<GPTAssistantAPIFunctionVO>().module_name, ModuleOseliaServer.getInstance().name)
            .filter_by_text_eq(field_names<GPTAssistantAPIFunctionVO>().module_function, reflect<ModuleOseliaServer>().user_mem_get_keys)
            .exec_as_server()
            .select_vo<GPTAssistantAPIFunctionVO>();
        if (!user_mem_get_keys_function) {
            user_mem_get_keys_function = new GPTAssistantAPIFunctionVO();

            user_mem_get_keys_function.archived = false;
            user_mem_get_keys_function.module_function = reflect<ModuleOseliaServer>().user_mem_get_keys;
            user_mem_get_keys_function.module_name = ModuleOseliaServer.getInstance().name;
            user_mem_get_keys_function.prepend_thread_vo = true;
            user_mem_get_keys_function.gpt_function_name = reflect<ModuleOseliaServer>().user_mem_get_keys;
            user_mem_get_keys_function.json_stringify_output = false;
            user_mem_get_keys_function.gpt_function_description = "Avant de répondre à un utilisateur et pour avoir la réponse la plus adaptée à cet utilisateur, cette fonction permet de charger les clés de la mémoire liée à un utilisateur correpondantes à une regexp passée en paramètre. Sans regexp, on renvoie toutes les clés de la mémoire utilisateur. A faire avant de charger les entrées de la mémoire pour cibler les entrées pertinentes. Cette mémoire a pour but de définir la relation avec l'utilisateur, comme le tutoiement/vouvoiement, le style de réponse, le ton, etc.";
            await ModuleDAOServer.instance.insertOrUpdateVO_as_server(user_mem_get_keys_function);
        }
        ...
        let argument_pattern = await query(GPTAssistantAPIFunctionParamVO.API_TYPE_ID)
            ...
            .filter_by_id(user_mem_get_keys_function.id, GPTAssistantAPIFunctionVO.API_TYPE_ID)
            .exec_as_server()
        ...
        if (!argument_pattern) {
            ...
            argument_pattern.function_id = user_mem_get_keys_function.id;
        ...
        if (!argument_user_id) {
            ...
            argument_user_id.function_id = user_mem_get_keys_function.id;
        ...
        if (!argument_asked_by) {
            ...
            argument_asked_by.function_id = user_mem_get_keys_function.id;
