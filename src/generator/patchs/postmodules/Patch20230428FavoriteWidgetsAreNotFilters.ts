...
export default class Patch20230428FavoriteWidgetsAreNotFilters implements IGeneratorWorker {
    ...
    public async work(db: IDatabase<any>) {
        ...
        const WIDGET_NAME_showfavoritesfilters = await query(DashboardWidgetVO.API_TYPE_ID).filter_by_text_eq('name', DashboardWidgetVO.WIDGET_NAME_showfavoritesfilters).select_vo<DashboardWidgetVO>();
        ...
        const WIDGET_NAME_savefavoritesfilters = await query(DashboardWidgetVO.API_TYPE_ID).filter_by_text_eq('name', DashboardWidgetVO.WIDGET_NAME_savefavoritesfilters).select_vo<DashboardWidgetVO>();
