...
export default class Patch20231030FilePathUnique implements IGeneratorWorker {
    ...
    public static getInstance(): Patch20231030FilePathUnique {
        ...
        return Patch20231030FilePathUnique.instance;
    }

    private static instance: Patch20231030FilePathUnique = null;

    get uid(): string {
        return 'Patch20231030FilePathUnique';
    }
        ...
        try {
            ...
            if (path_rows && path_rows.length) {
                ...
                for (let i in paths) {
                    ...
                    // delete throwed ids
                    await db.none("delete from ref.module_file_file where id in (" + throwed_ids.join(',') + ")");
                }
            }
            ...
            WITH Ranked AS (
                ...
                FROM ref.module_file_file
            )
            DELETE FROM ref.module_file_file
            WHERE id IN (
                SELECT id
                FROM Ranked
                WHERE rn > 1
            ...
            DO $$
                ...
                BEGIN
                    ...
                    SELECT EXISTS (
                        ...
                        AND    t.relname = 'module_file_file'  -- Nom de la table
                    ...
                    IF NOT v_constraint_exists THEN
                        EXECUTE 'ALTER TABLE ref.module_file_file ADD UNIQUE (path);';
                    END IF;
            END $$;
            `);
