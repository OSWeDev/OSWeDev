...
export default abstract class VueAppController {
    ...
    public async initialize() {
        ...
        promises.push((async () => {
            ...
            const user_lang = self.data_user_lang ? self.data_user_lang.code_lang : null;
    ...
    private try_language(code_lang: string): string {
        ...
        const code_lang_separator = (code_lang.indexOf('-') > 0) ? '-' : '_';

        let exact = null;
        let start_exact = null;
        const code_lang_start = (code_lang && (code_lang.indexOf(code_lang_separator) > 0)) ? code_lang.split(code_lang_separator)[0] : code_lang;
        for (const i in this.ALL_LANGS) {
            const lang = this.ALL_LANGS[i];
            ...
            const lang_code_lang_separator = (lang.code_lang.indexOf('-') > 0) ? '-' : '_';
            const lang_start = (lang.code_lang && (lang.code_lang.indexOf(lang_code_lang_separator) > 0)) ? lang.code_lang.split(lang_code_lang_separator)[0] : lang.code_lang;
