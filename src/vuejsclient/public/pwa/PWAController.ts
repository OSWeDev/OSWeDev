...
export default class PWAController {
    ...
    public push_subscription: PushSubscription = null;
    ...
    public async add_pwa_subscription(user_id: number) {
        if (!user_id || !this.push_subscription) {
            return;
        }
        ...
        const push_subscription_json: PushSubscriptionJSON = this.push_subscription.toJSON();
