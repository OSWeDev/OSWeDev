...
@Component({
    ...
    components: {
        choroplethchart: VueChart,
    }
})
export default class VarChoroplethChartComponent extends VueComponentBase {
    @ModuleVarGetter
    ...
    @Prop({ default: null })
    public var_dataset_descriptor: VarChoroplethDataSetDescriptor;
    ...
    private throttled_update_chart_js = ThrottleHelper.declare_throttle_without_args(
        'VarChoroplethChartComponent.throttled_update_chart_js',
    ...
    private throttled_var_datas_updater = ThrottleHelper.declare_throttle_without_args(
        'VarChoroplethChartComponent.throttled_var_datas_updater',
    ...
    private render_chart_js() {
        ...
        if (this.rendered) {
            ConsoleHandler.error('PB:render Choropleth Chart déjà rendu');
            return;
        }
        ...
        try {
            ...
            // ConsoleHandler.warn('PB:render Choropleth Chart probablement trop tôt:' + error);
            ConsoleHandler.error('PB:render Choropleth Chart probablement trop tôt:' + error + ':');
            this.rendered = false;
