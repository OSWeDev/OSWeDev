    ...
    private loadDatasFromDatatableField(load_from_datatable_field: DatatableField<any, any>): Array<Promise<any>> {
        ...
        if ((load_from_datatable_field.type == DatatableField.MANY_TO_ONE_FIELD_TYPE) ||
            ...
            for (const i in reference.sorted_target_fields) {
                res = res.concat(
                    this.loadDatasFromDatatableField(reference.sorted_target_fields[i]),
                );
            }
        }
        ...
        for (const i in this.crud.createDatatable.fields) {
            ...
            switch (field.type) {
                case DatatableField.SIMPLE_FIELD_TYPE:
                    obj[field.datatable_field_uid] = ((obj[field.datatable_field_uid]) ? obj[field.datatable_field_uid] : await (field as SimpleDatatableFieldVO<any, any>).moduleTableField.get_field_default_value());
                    break;
                case DatatableField.MANY_TO_ONE_FIELD_TYPE:
                    obj[field.datatable_field_uid] = ((obj[field.datatable_field_uid]) ? obj[field.datatable_field_uid] : await (field as ManyToOneReferenceDatatableFieldVO<any>).srcField.get_field_default_value());
                    break;
                case DatatableField.REF_RANGES_FIELD_TYPE:
                    obj[field.datatable_field_uid] = ((obj[field.datatable_field_uid]) ? obj[field.datatable_field_uid] : await (field as RefRangesReferenceDatatableFieldVO<any>).srcField.get_field_default_value());
                    break;
