<template lang="pug">
    ...
    .field(v-if="((field.type == 'INPUT') || (field.datatable_field_uid == field.module_table_field_id)) && (field.isVisibleUpdateOrCreate(vo))", :field="field.datatable_field_uid", :field_type="field_type")
...
export default class CRUDComponentField extends VueComponentBase
  ...
  @ModuleDAOGetter
  public getStoredDatas: {
    [API_TYPE_ID: string]: { [id: number]: IDistantVOBase };
  };
  @ModuleDAOAction
  private storeDatasByIds: (params: {
    API_TYPE_ID: string;
    vos_by_ids: { [id: number]: IDistantVOBase };
  }) => void;
  ...
  get is_segmented_day_tsrange_array() {
    ...
    if (!!field) {
      return (
        ...
        field.segmentation_type == TimeSegment.TYPE_DAY
      );
    }
  }
  ...
  private async reload_field_value() {
    ...
    this.field_value =
      this.vo && this.field ? this.vo[this.field.datatable_field_uid] : null;
    ...
    if (
      this.field_type == ModuleTableField.FIELD_TYPE_daterange &&
      this.field_value
    ) {
      ...
      if (date && date.length > 0) {
        Vue.set(
          this.field_value_range,
          this.field.datatable_field_uid + "_start",
          this.formatDateForField(date[0])
        );
        Vue.set(
          this.field_value_range,
          this.field.datatable_field_uid + "_end",
          this.formatDateForField(date[1])
        );
      }
    }
      ...
      ModuleAccessPolicy.getInstance()
        .checkAccess(
          ModuleDAO.getInstance().getAccessPolicyName(
            ...
            (this.field as ReferenceDatatableField<any>).targetModuleTable
              .vo_type
          )
        )
        .then((res: boolean) => {
    ...
    if (this.field.type == DatatableField.REF_RANGES_FIELD_TYPE) {
      ...
      if (
        !this.select_options ||
        RangeHandler.getInstance().getCardinalFromArray(this.field_value) >
        ...
        for (let i in this.select_options) {
          ...
          if (
            RangeHandler.getInstance().elt_intersects_any_range(
              id,
              this.field_value
            )
          ) {
        ...
        let options_by_id: {
          [id: number]: boolean;
        } = ObjectHandler.getInstance().mapFromIdsArray(this.select_options);
  ...
  private formatDateForField(date: string, separator: string = "/"): string {
    ...
    return DateHandler.getInstance().formatDayForIndex(
      moment()
        .utc(true)
        .year(parseInt(dateCut[2]))
        .month(parseInt(dateCut[1]) - 1)
        .date(parseInt(dateCut[0]))
    );
  }
  ...
  private validateInput(input: any) {
    ...
    if (
      ...
      (this.field as SimpleDatatableField<any, any>).moduleTableField
        .field_type == ModuleTableField.FIELD_TYPE_html
    ) {
      input_value = input;
    } else {
    ...
    if (
      this.field.type == DatatableField.SIMPLE_FIELD_TYPE &&
      (this.field as SimpleDatatableField<any, any>).moduleTableField
        .field_type == ModuleTableField.FIELD_TYPE_boolean &&
            ...
            switch (
              ...
              default:
                input.setCustomValidity
                  ? input.setCustomValidity(
                      this.label(ModuleTableField.VALIDATION_CODE_TEXT_required)
                    )
                  : document
                      .getElementById(input.id)
                      ["setCustomValidity"](
                        this.label(
                          ModuleTableField.VALIDATION_CODE_TEXT_required
                        )
                      );
                return;
            }
    ...
    if (
      ...
      input.setCustomValidity
        ...
        : document.getElementById(input.id)["setCustomValidity"](msg);
    }
  ...
  private changeValue(
    ...
    if (!this.datatable) {
      vo[field.datatable_field_uid] = this.field.UpdateIHMToData(
        value,
        this.vo
      );
    } else {
    ...
    for (let i in datatable.fields) {
      ...
      if (field_datatable.type == DatatableField.MANY_TO_ONE_FIELD_TYPE) {
        let manyToOneField: ManyToOneReferenceDatatableField<any> = field_datatable as ManyToOneReferenceDatatableField<
          any
        >;
        let options = this.getStoredDatas[
          manyToOneField.targetModuleTable.vo_type
        ];

        if (!!manyToOneField.filterOptionsForUpdateOrCreateOnManyToOne) {
          options = manyToOneField.filterOptionsForUpdateOrCreateOnManyToOne(
            vo,
            options
          );
        }
        ...
        if (options) {
          field_datatable.setSelectOptionsEnabled(
            ObjectHandler.getInstance()
              .arrayFromMap(options)
              .map(elem => elem.id)
          );
        }
      }
      ...
      if (field_datatable.type == DatatableField.REF_RANGES_FIELD_TYPE) {
        let refrangesField: RefRangesReferenceDatatableField<any> = field_datatable as RefRangesReferenceDatatableField<
          any
        >;
        let options = this.getStoredDatas[
          refrangesField.targetModuleTable.vo_type
        ];

        if (!!refrangesField.filterOptionsForUpdateOrCreateOnRefRanges) {
          options = refrangesField.filterOptionsForUpdateOrCreateOnRefRanges(
            vo,
            options
          );
        }
  ...
  private updateDateRange(input: any) {
    ...
    let start = this.field_value_range[
      this.field.datatable_field_uid + "_start"
    ];
    ...
    if (start) {
      try {
        res += ModuleFormatDatesNombres.getInstance().formatDate_FullyearMonthDay(
  ...
  private async prepare_select_options() {
    if (
      ...
      let manyToOne: ReferenceDatatableField<any> = this
        .field as ReferenceDatatableField<any>;
      ...
      if (!ObjectHandler.getInstance().hasAtLeastOneAttribute(options)) {
        options = VOsTypesManager.getInstance().vosArray_to_vosByIds(
          await ModuleDAO.getInstance().getVos(
            manyToOne.targetModuleTable.vo_type
          )
        );
        this.storeDatasByIds({
          API_TYPE_ID: manyToOne.targetModuleTable.vo_type,
          vos_by_ids: options
        });
      }

      if (this.field.type == DatatableField.MANY_TO_ONE_FIELD_TYPE) {
        let manyToOneField: ManyToOneReferenceDatatableField<any> = this
          .field as ManyToOneReferenceDatatableField<any>;
        if (!!manyToOneField.filterOptionsForUpdateOrCreateOnManyToOne) {
          options = manyToOneField.filterOptionsForUpdateOrCreateOnManyToOne(
            this.vo,
            options
      ...
      if (this.field.type == DatatableField.REF_RANGES_FIELD_TYPE) {
        let refRangesReferenceDatatableField: RefRangesReferenceDatatableField<any> = this
          .field as RefRangesReferenceDatatableField<any>;
        if (
          !!refRangesReferenceDatatableField.filterOptionsForUpdateOrCreateOnRefRanges
        ) {
          options = refRangesReferenceDatatableField.filterOptionsForUpdateOrCreateOnRefRanges(
            this.vo,
            options
          );
        }
      ...
      for (let j in options) {
        ...
        if (
          !this.select_options_enabled ||
          this.select_options_enabled.indexOf(option.id) >= 0
        ) {
    ...
    if (this.field.type == DatatableField.SIMPLE_FIELD_TYPE) {
      let simpleField: SimpleDatatableField<any, any> = this
        .field as SimpleDatatableField<any, any>;

      if (
        simpleField.moduleTableField.field_type ==
        ...
        for (let j in simpleField.moduleTableField.enum_values) {
          ...
          if (
            !this.select_options_enabled ||
            this.select_options_enabled.indexOf(id) >= 0
          ) {
            newOptions.push(id);
          }
        }
  ...
  private asyncLoadEnumOptions(query: string) {
    ...
    if (!this.field || this.field.type != DatatableField.SIMPLE_FIELD_TYPE) {
      this.snotify.warning(this.label("crud.multiselect.search.error"));
      this.isLoadingOptions = false;
      return;
    }
    ...
    for (let i in simpleField.moduleTableField.enum_values) {
      if (
        simpleField
          .enumIdToHumanReadable(parseInt(i))
          .toLowerCase()
          .indexOf(query.toLowerCase()) >= 0
      ) {
        if (
          !this.select_options_enabled ||
          this.select_options_enabled.indexOf(parseInt(i)) >= 0
        ) {
  ...
  private async onChangeField() {
    ...
    if (this.field_type == DatatableField.REF_RANGES_FIELD_TYPE) {
      ...
      for (let i in this.field_value_refranges_selected_ids) {
        let id = parseInt(
          this.field_value_refranges_selected_ids[i].toString()
        );

        ranges.push(
          RangeHandler.getInstance().create_single_elt_NumRange(
            id,
            NumSegment.TYPE_INT
          )
        );
      ...
      let refrangesField: RefRangesReferenceDatatableField<any> = this
        .field as RefRangesReferenceDatatableField<any>;
      ...
      if (!ObjectHandler.getInstance().hasAtLeastOneAttribute(options)) {
        ...
        this.storeDatasByIds({
          API_TYPE_ID: refrangesField.targetModuleTable.vo_type,
          vos_by_ids: options
        });
      }
    ...
    if (this.field.type == DatatableField.MANY_TO_ONE_FIELD_TYPE) {
      ...
      if (!ObjectHandler.getInstance().hasAtLeastOneAttribute(options)) {
        ...
        this.storeDatasByIds({
          API_TYPE_ID: manyToOneField.targetModuleTable.vo_type,
          vos_by_ids: options
        });
      }
  ...
  get custom_field_types(): { [name: string]: TableFieldTypeControllerBase } {
    return TableFieldTypesManager.getInstance()
      .registeredTableFieldTypeControllers;
  }

  get field_type(): string {
    if (this.field.type == "Simple") {
      return (this.field as SimpleDatatableField<any, any>).moduleTableField
        .field_type;
    }
  ...
  private async select_all() {
    switch (this.field.type) {
      case DatatableField.REF_RANGES_FIELD_TYPE:
        this.field_value_refranges_selected_ids = Array.from(
          this.select_options
        );
        break;
      case DatatableField.MANY_TO_MANY_FIELD_TYPE:
  ...
  private async inline_clear_value() {
    ...
    this.field_value = this.field.dataToUpdateIHM(
      this.inline_input_read_value,
      this.vo
    );
  }

  private async validate_inline_input() {
    let alerts: Alert[] = this.field.validate_input
      ? this.field.validate_input(this.field_value, this.field, this.vo)
      : null;
  ...
  private async change_inline_field_value() {
    ...
    if (this.auto_update_field_value) {
      ...
      this.vo[this.field.datatable_field_uid] = this.field.UpdateIHMToData(
        this.field_value,
        this.vo
      );

      let result: InsertOrDeleteQueryResult = await ModuleDAO.getInstance().insertOrUpdateVO(
        this.vo
      );

      if (!result || !result.id) {
        await this.snotify.error(
          this.label("field.auto_update_field_value.failed")
        );
        ...
        this.register_alert(
          new Alert(
            this.alert_path,
            "field.auto_update_field_value.server_error"
          )
        );
        ...
        await this.snotify.success(
          this.label("field.auto_update_field_value.succes")
        );
      }
    }
  ...
  get targetModuleTable_count(): number {
    ...
    if (
      manyToOne &&
      manyToOne.targetModuleTable &&
      manyToOne.targetModuleTable.vo_type &&
      this.getStoredDatas &&
      this.getStoredDatas[manyToOne.targetModuleTable.vo_type]
    ) {
      return ObjectHandler.getInstance().arrayFromMap(
        this.getStoredDatas[manyToOne.targetModuleTable.vo_type]
      ).length;
    }
...
.field {
  ...
  .inline_input_show_clear,
  .inline_input_is_editing,
  .inline_input_cancel_input,
  .inline_input_is_not_editing {
    padding: 5px;
    height: 1.8em;
    padding-top: 0.3em;
    font-size: 1em;
  ...
  .inline_input_show_clear.busy,
  .inline_input_is_editing.busy,
  .inline_input_cancel_input.busy,
  .inline_input_is_not_editing.busy {
    opacity: 0.5;
    color: #888 !important;
