...
export default class BardChatroomHandlerComponent extends VueComponentBase {
    ...
    private reload() {
        ...
        // this.tmp_type = '' + BardChatroomVO.FEEDBACK_TYPE_NOT_SET;
    ...
    private async send_feedback() {
        ...
        StatsController.register_stat_COMPTEUR("BardChatroomHandlerComponent", "send_feedback", "IN");
        ...
        if (this.is_already_sending_feedback) {
            StatsController.register_stat_COMPTEUR("BardChatroomHandlerComponent", "send_feedback", "ALREADY_SENDING");
            return;
        }
        ...
        if ((!this.tmp_message) || (!this.tmp_title)) {
            StatsController.register_stat_COMPTEUR("BardChatroomHandlerComponent", "send_feedback", "ERROR_NO_MESSAGE_OR_TITLE");
            this.snotify.error(this.label('BardChatroomHandlerComponent.needs_message_and_title'));
            this.is_already_sending_feedback = false;
            return;
        }

        if ((!this.tmp_user) || (!this.tmp_email)) {
            StatsController.register_stat_COMPTEUR("BardChatroomHandlerComponent", "send_feedback", "ERROR_NO_USER_OR_EMAIL");
            this.snotify.error(this.label('BardChatroomHandlerComponent.needs_user_and_email'));
            this.is_already_sending_feedback = false;
            return;
        }

        if ((!this.tmp_capture_1_vo) || (!this.tmp_capture_1_vo.id)) {
            StatsController.register_stat_COMPTEUR("BardChatroomHandlerComponent", "send_feedback", "ERROR_NO_SCREENSHOT");
            this.snotify.error(this.label('BardChatroomHandlerComponent.needs_at_least_one_screenshot'));
            this.is_already_sending_feedback = false;
            return;
        }

        if (this.tmp_wish_be_called && (!this.tmp_phone || !this.tmp_preferred_times_called)) {
            StatsController.register_stat_COMPTEUR("BardChatroomHandlerComponent", "send_feedback", "ERROR_NO_PHONE");
            this.snotify.error(this.label('BardChatroomHandlerComponent.needs_phone'));
            this.is_already_sending_feedback = false;
            return;
        }

        let feedback: BardChatroomVO = new BardChatroomVO();
        ...
        if (!await ModuleBardChatroom.getInstance().feedback(feedback)) {
            this.set_hidden(true);

            StatsController.register_stat_DUREE("BardChatroomHandlerComponent", "send_feedback", "ERROR_SENDING_FEEDBACK", Dates.now_ms() - time_in);
            StatsController.register_stat_COMPTEUR("BardChatroomHandlerComponent", "send_feedback", "ERROR_SENDING_FEEDBACK");
            this.snotify.error(this.label('BardChatroomHandlerComponent.error_sending_feedback'));
            this.is_already_sending_feedback = false;
            return;
        }
        ...
        this.is_already_sending_feedback = false;
        this.reload();
        StatsController.register_stat_DUREE("BardChatroomHandlerComponent", "send_feedback", "OUT", Dates.now_ms() - time_in);
    }
    ...
    private check_for_valid_format(path: string) {
        ...
        if (format.test(file_name) || (file_name.indexOf(' ') >= 0)) {
            this.set_hidden(true);
            this.snotify.error(this.label('BardChatroomHandlerComponent.file_format_error'));
            return false;
        }
        return true;
    }
    ...
    get isActive(): boolean {
        return ModuleBardChatroom.getInstance().actif && VueAppController.getInstance().has_access_to_feedback;
    }
}