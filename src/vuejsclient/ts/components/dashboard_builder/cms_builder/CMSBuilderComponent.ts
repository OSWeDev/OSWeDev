...
import DashboardMenuConfComponent from '../../dashboard_builder/menu_conf/DashboardMenuConfComponent';
...
export default class CMSBuilderComponent extends VueComponentBase {
    ...
    private async create_dashboard_page() {
        ...
        if ((!insertOrDeleteQueryResult) || (!insertOrDeleteQueryResult.id)) {
            this.snotify.error(this.label('CMSBuilderComponent.create_new_dashboard.ko'));
            this.dashboard = null;
            return;
        }
    ...
    private async confirm_delete_dashboard() {
        ...
        if (this.dashboards.length <= 1) {
            self.snotify.error(self.label('CMSBuilderComponent.delete_dashboard.cannot_delete_master_dashboard'));
            return;
        }

        // On demande confirmation avant toute chose.
        // si on valide, on lance la suppression
        self.snotify.confirm(self.label('CMSBuilderComponent.delete_dashboard.confirmation.body'), self.label('CMSBuilderComponent.delete_dashboard.confirmation.title'), {
            timeout: 10000,
            showProgressBar: true,
            closeOnClick: false,
            pauseOnHover: true,
            buttons: [
                {
                    ...
                    action: async (toast) => {
                        ...
                        self.snotify.info(self.label('CMSBuilderComponent.delete_dashboard.start'));
                        ...
                        self.snotify.success(self.label('CMSBuilderComponent.delete_dashboard.ok'));
                    },
                    bold: false,
                },
                {
    ...
    private async confirm_delete_page(page: DashboardPageVO) {
        ...
        if (this.pages.length <= 1) {
            self.snotify.error(self.label('CMSBuilderComponent.delete_page.cannot_delete_master_page'));
            return;
        }

        // On demande confirmation avant toute chose.
        // si on valide, on lance la suppression
        self.snotify.confirm(self.label('CMSBuilderComponent.delete_page.confirmation.body'), self.label('CMSBuilderComponent.delete_page.confirmation.title'), {
            timeout: 10000,
            showProgressBar: true,
            closeOnClick: false,
            pauseOnHover: true,
            buttons: [
                {
                    ...
                    action: async (toast) => {
                        self.$snotify.remove(toast.id);
                        self.snotify.info(self.label('CMSBuilderComponent.delete_page.start'));
                        ...
                        self.snotify.success(self.label('CMSBuilderComponent.delete_page.ok'));
                    },
                    bold: false,
                },
                {
        ...
        if (set_position_default) {
            ...
            for (const i in positions) {
                ...
                if (default_positions[i]) {
                    ...
                    pos.h = default_positions[i] ? default_positions[i].h : null;
                }
    ...
    private async mounted() {
        ...
        this.show_cms_dashboard_pages = await ModuleParams.getInstance().getParamValueAsBoolean(ModuleDashboardBuilder.PARAM_NAME_SHOW_CMS_DASHBOARD_PAGES, null, null);
