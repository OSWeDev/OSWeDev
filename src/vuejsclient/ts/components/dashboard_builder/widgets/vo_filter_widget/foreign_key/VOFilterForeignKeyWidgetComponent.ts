...
import './VOFilterForeignKeyWidgetComponent.scss';

@Component({
    template: require('./VOFilterForeignKeyWidgetComponent.pug'),
    components: {
    }
})
export default class VOFilterForeignKeyWidgetComponent extends VueComponentBase {
    ...
    private advanced_foreign_key_filters: AdvancedNumberFilter[] = [new AdvancedNumberFilter()];

    private warn_existing_external_filters: boolean = false;

    private is_init: boolean = false;
    private old_widget_options: VOFilterWidgetOptionsVO = null;
    ...
    private filter_type_options: number[] = [
        NumberConditionStatementValue[NumberConditionStatementLabel.LOWER_THAN],
        NumberConditionStatementValue[NumberConditionStatementLabel.LOWER_THAN_OR_EQUALS],
        NumberConditionStatementValue[NumberConditionStatementLabel.GREATER_THAN],
        NumberConditionStatementValue[NumberConditionStatementLabel.GREATER_THAN_OR_EQUALS],
        NumberConditionStatementValue[NumberConditionStatementLabel.IS_NULL],
        NumberConditionStatementValue[NumberConditionStatementLabel.IS_NOT_NULL],
        NumberConditionStatementValue[NumberConditionStatementLabel.EQUALS],
        NumberConditionStatementValue[NumberConditionStatementLabel.NOT_EQUALS],
    ];
    ...
    private onchange_advanced_foreign_key_filter_content() {
        if (this.autovalidate_advanced_filter) {
            this.validate_advanced_foreign_key_filter();
        }
    }

    private add_advanced_foreign_key_filter() {
        if (!this.advanced_foreign_key_filters) {
            this.advanced_foreign_key_filters = [];
            return;
        }

        this.advanced_foreign_key_filters[this.advanced_foreign_key_filters.length - 1].link_type = AdvancedNumberFilter.LINK_TYPE_ET;
        this.advanced_foreign_key_filters.push(new AdvancedNumberFilter());
    }

    private validate_advanced_foreign_key_filter() {
        ...
        for (let i in this.advanced_foreign_key_filters) {
            let advanced_filter: AdvancedNumberFilter = this.advanced_foreign_key_filters[i];
    ...
    private delete_advanced_foreign_key_filter(i: number) {
        if ((!this.advanced_foreign_key_filters) || (i >= this.advanced_foreign_key_filters.length - 1)) {
            return;
        }
        this.advanced_foreign_key_filters.splice(i, 1);
    }

    private switch_link_type(advanced_foreign_key_filter: AdvancedNumberFilter) {
        advanced_foreign_key_filter.link_type = 1 - advanced_foreign_key_filter.link_type;
    }
    ...
    private async switch_advanced_filters() {
        ...
        this.advanced_foreign_key_filters = [new AdvancedNumberFilter()];
    ...
    private async update_visible_options() {
        ...
        let context_query = query(api_type_id).set_limit(
            this.widget_options.max_visible_options,
            0
        );
        context_query.fields = [new ContextQueryFieldVO(this.vo_field_ref.api_type_id, this.vo_field_ref.field_id, 'label')];
        context_query.filters = ContextFilterVOManager.get_context_filters_from_active_field_filters(active_field_filters_query);
        context_query.active_api_type_ids = this.dashboard.api_type_ids;

        VOFilterWidgetController.getInstance().add_discarded_field_paths(context_query, this.get_discarded_field_paths);
        ...
        if (
            ...
            context_query = await VOFilterWidgetController.getInstance().check_segmented_dependencies(
                this.dashboard,
                context_query,
    ...
    private try_apply_actual_active_filters(filter: ContextFilterVO): boolean {
        ...
        if (this.has_advanced_filter(filter)) {
            ...
            this.advanced_foreign_key_filters = advanced_filters;
        } else {
    ...
    get is_advanced_filter_valid(): boolean {
        ...
        if ((!this.advanced_foreign_key_filters) || (!this.advanced_foreign_key_filters.length)) {
            return false;
        }

        for (let i in this.advanced_foreign_key_filters) {
            let advanced_foreign_key_filter = this.advanced_foreign_key_filters[i];

            if (!this.has_content_filter_type[advanced_foreign_key_filter.filter_type]) {
                continue;
            }

            if (advanced_foreign_key_filter.filter_content == null) {
                return false;
            }
        }
