...
export default class CurrentUserFilterWidgetComponent extends VueComponentBase {
    ...
    private async onchange_widget_options(): Promise<void> {
        ...
        const context_filter = ContextFilterVOManager.create_context_filter_from_data_filter_option(
            this.create_data_filter(this.vo_field_ref.field_id),
            null,
            this.field,
            this.vo_field_ref
        );
    ...
    private create_data_filter(field_id: string): DataFilterOption {
        let val: number = this.user[field_id];
        ...
        data_filter.string_value = this.field?.field_value; // case of name
