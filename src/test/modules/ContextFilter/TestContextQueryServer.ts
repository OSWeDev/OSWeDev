...
describe('ContextQueryServer', () => {
    ...
    it('test .build_select_query having auto', async () => {
        ...
        context_query.add_filters([
            filter(RoleVO.API_TYPE_ID).by_id_in(query(RoleVO.API_TYPE_ID).field('id'), context_query).or(
                filter(UserVO.API_TYPE_ID).by_id_not_in(query(UserRoleVO.API_TYPE_ID).field('user_id').exec_as_admin(), context_query)
            )
        ]).exec_as_admin();
    ...
    it('test .build_select_query having auto simple', async () => {
        ...
        let context_query: ContextQueryVO = query(RoleVO.API_TYPE_ID).filter_by_id(15, UserVO.API_TYPE_ID).exec_as_admin();
    ...
    it('test .build_select_query chained OR', async () => {
        ...
        let f1 = ContextFilterVO.or([
            filter(RoleVO.API_TYPE_ID).by_id_in(query(RoleVO.API_TYPE_ID).field('id').exec_as_admin(), context_query),
            filter(UserVO.API_TYPE_ID).by_id_not_in(query(UserRoleVO.API_TYPE_ID).field('user_id').exec_as_admin(), context_query),
        ...
        let compl = ContextFilterVO.or([
            filter(UserVO.API_TYPE_ID).by_id_in(
                query(UserVO.API_TYPE_ID).field('id').exec_as_admin(), context_query),
            filter(UserVO.API_TYPE_ID).by_id_in(
                query(UserVO.API_TYPE_ID).field('id').exec_as_admin(), context_query)
        ]);
        filter_ = filter_.or(compl);

        context_query.add_filters([f1]).add_filters([filter_]).exec_as_admin();
