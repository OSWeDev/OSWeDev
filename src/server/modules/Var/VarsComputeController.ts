...
export default class VarsComputeController {
    ...
    public get_translatable_name_code(varConf_id: number): string {
        return VarsComputeController.VARS_DESC_TRANSLATABLE_PREFIXES + this.getVarConfById(varConf_id).name + '.translatable_name' + DefaultTranslation.DEFAULT_LABEL_EXTENSION;
    }

    public get_translatable_description_code(varConf_id: number): string {
        return VarsComputeController.VARS_DESC_TRANSLATABLE_PREFIXES + this.getVarConfById(varConf_id).name + '.translatable_description' + DefaultTranslation.DEFAULT_LABEL_EXTENSION;
    }

    public get_translatable_params_desc_code(varConf_id: number): string {
        return VarsComputeController.VARS_DESC_TRANSLATABLE_PREFIXES + this.getVarConfById(varConf_id).name + '.translatable_params_desc' + DefaultTranslation.DEFAULT_LABEL_EXTENSION;
    }
    ...
    public clean_caches_and_vardag() {
        ...
        VarsComputeController.getInstance().varDAG.clearDAG();
    ...
    public get_tsrange_on_resetable_var(var_id: number, target: Moment, min_inclusiv: boolean, max_inclusiv: boolean, segment_type: number): TSRange {
        ...
        let controller = VarsComputeController.getInstance().getVarControllerById(var_id);
        ...
        for (let i in nodes_to_request) {
            ...
            promises.push((async () => {
                ...
                if (var_controller.can_use_optimized_imports_calculation && ne_peut_pas_calculer) {
                    ...
                    VarsComputeController.getInstance().setVarData(var_value, true);
    ...
    private updateDepssAfterImport(nodes: { [node_name: string]: VarDAGNode }) {
        ...
        for (let i in nodes) {
            ...
            let node_controller = VarsComputeController.getInstance().getVarControllerById(node.param.var_id);
            ...
            if (!all_ok) {
                ...
                if (this.varDAG.marked_nodes_names[VarDAG.VARDAG_MARKER_NEEDS_PREDEPS_DATASOURCE_LOADING] &&
                    ...
                    for (let i in nodes_names_to_preload) {
                        ...
                        let datasources_predeps: Array<DataSourceControllerBase<any>> = VarsComputeController.getInstance().getVarControllerById(varDagNode.param.var_id).getDataSourcesPredepsDependencies();
    ...
    private computeNode(node: VarDAGNode) {
        ...
        while (continue_compilation) {
            ...
            VarsComputeController.getInstance().getVarControllerById(actual_node.param.var_id).computeValue(actual_node, this.varDAG);
