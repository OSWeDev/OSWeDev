...
export default class CachedinfoCronWorker implements ICronWorker {
    ...
    public static getInstance() {
        if (!CachedinfoCronWorker.instance) {
            CachedinfoCronWorker.instance = new CachedinfoCronWorker();
        }
        return CachedinfoCronWorker.instance;
    }

    private static instance: CachedinfoCronWorker = null;

    private constructor() {
    }

    get worker_uid(): string {
        return "CachedinfoCronWorker";
    }
    ...
    public async work() {
        ...
        for (let i in VarsController.getInstance().registered_vars_controller) {
            ...
            if (!VOsTypesManager.getInstance().moduleTables_by_voType[controller.varConf.var_data_vo_type].isMatroidTable) {
                ConsoleHandler.getInstance().error('CachedinfoCronWorker:var_id:' + controller.varConf.id + ':name:' + controller.varConf.name + ':!isMatroidTable');
                continue;
            }

            let dss = controller.getDataSourcesDependencies();
            if ((!dss) || (dss.length <= 1)) {
                ConsoleHandler.getInstance().error('CachedinfoCronWorker:var_id:' + controller.varConf.id + ':name:' + controller.varConf.name + ':!dss>1');
                continue;
            }
            ...
            if (deps_ids && deps_ids.length) {
                ConsoleHandler.getInstance().error('CachedinfoCronWorker:var_id:' + controller.varConf.id + ':name:' + controller.varConf.name + ':deps_ids!=0');
                continue;
            }
            ...
            // if (refuse) {
            //     ConsoleHandler.getInstance().error('CachedinfoCronWorker:var_id:' + controller.varConf.id + ':name:' + controller.varConf.name + ':deps cached');
            //     continue;
            // }
            ...
            if (refuse) {
                ConsoleHandler.getInstance().error('CachedinfoCronWorker:var_id:' + controller.varConf.id + ':name:' + controller.varConf.name + ':parents cached');
                continue;
            }
        }
    }
