...
export default class PushDataServerController {
     ...
     */
    public async notifyUserLoggedAndRedirectHome(redirect_uri: string, sso: boolean) {

        // Permet d'assurer un lancement uniquement sur le main process
        if (!await ForkedTasksController.exec_self_on_main_process(PushDataServerController.TASK_NAME_notifyUserLoggedAndRedirectHome, redirect_uri, sso)) {
            return;
        }
