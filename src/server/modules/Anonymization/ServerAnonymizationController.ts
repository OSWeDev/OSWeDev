...
import PushDataServerController from "../PushData/PushDataServerController";
import ServerAnonymizationReloadConfMessage from "./vos/ServerAnonymizationReloadConfMessage";
...
//         const conf = ServerAnonymizationController.registered_anonymization_field_conf_by_id[anonymization_user_conf.anon_field_name];
...
//                 for (const field_id in ServerAnonymizationController.registered_anonymization_field_conf_by_vo_type_and_field_id[vo._type]) {
...
//             const api_type_id = field.api_type_id;
//             const field_id = field.field_name;
...
//             const anonymization_field_conf = ServerAnonymizationController.registered_anonymization_field_conf_by_vo_type_and_field_id[row_field_api_type_id][row_field_field_id];
...
//              */
//             const known_values_by_id: { [before_anonymization: string]: string } = ServerAnonymizationController.registered_anonymization_values[row_field_api_type_id][row_field_field_id];

//             if (known_values_by_id) {

//                 for (const i in known_values_by_id) {
//                     const known_value = known_values_by_id[i];
...
//             const before_anonymization = row[alias];
...
//         const fields_confs: AnonymizationFieldConfVO[] = await query(AnonymizationFieldConfVO.API_TYPE_ID).select_vos<AnonymizationFieldConfVO>();
//         const users_confs: AnonymizationUserConfVO[] = await query(AnonymizationUserConfVO.API_TYPE_ID).select_vos<AnonymizationUserConfVO>();

//         // On est déjà sur un broadcast on a pas besoin de broadcast le register
//         for (const i in fields_confs) {
//             ServerAnonymizationController.register_anonymization_field_conf(fields_confs[i]);
//         }

//         for (const i in users_confs) {
...
export default class ServerAnonymizationController {
    ...
    public static registered_anonymization_field_conf_by_uid_vo_type_and_field_id: { [uid: number]: { [vo_type: string]: { [field_id: string]: AnonymizationFieldConfVO } } } = {};
    ...
    public static register_anonymization_user_conf(anonymization_user_conf: AnonymizationUserConfVO) {
        ...
        if (!ServerAnonymizationController.registered_anonymization_field_conf_by_uid_vo_type_and_field_id[anonymization_user_conf.user_id]) {
            ServerAnonymizationController.registered_anonymization_field_conf_by_uid_vo_type_and_field_id[anonymization_user_conf.user_id] = {};
        }

        const anon_conf = ServerAnonymizationController.registered_anonymization_field_conf_by_id[anonymization_user_conf.anon_field_name];
        if (!ServerAnonymizationController.registered_anonymization_field_conf_by_uid_vo_type_and_field_id[anonymization_user_conf.user_id][anon_conf.vo_type]) {
            ServerAnonymizationController.registered_anonymization_field_conf_by_uid_vo_type_and_field_id[anonymization_user_conf.user_id][anon_conf.vo_type] = {};
        }

        ServerAnonymizationController.registered_anonymization_field_conf_by_uid_vo_type_and_field_id[anonymization_user_conf.user_id][anon_conf.vo_type][anon_conf.field_name] = anon_conf;
    ...
    public static async reload_conf() {
        ...
        ServerAnonymizationController.registered_anonymization_field_conf_by_id = {};
        ServerAnonymizationController.registered_anonymization_field_conf_by_uid_vo_type_and_field_id = {};
    ...
    public static apply_anonymisation_conf_for_user_on_context_query(context_query: ContextQueryVO) {
        if (!context_query) {
            return;
        }

        // Si la conf est déjà appliquée, on ne la réapplique pas
        ...
        if (!ServerAnonymizationController.registered_anonymization_field_conf_by_uid_vo_type_and_field_id[uid]) {
            return;
        }

        const anonymization_user_confs: { [vo_type: string]: { [field_id: string]: boolean } } = {};
        const confs = ServerAnonymizationController.registered_anonymization_field_conf_by_uid_vo_type_and_field_id[uid];
        for (const api_type_id in confs) {
            anonymization_user_confs[api_type_id] = {};
            for (const field_name in confs[api_type_id]) {
                anonymization_user_confs[api_type_id][field_name] = true;
            }
        }

        context_query.anonimized_fields = anonymization_user_confs;
    }

    public static handle_anon_field_name(context_query: ContextQueryVO, api_type_id: string, field_name: string, full_request_field_name: string = null, do_not_happen_as: boolean = false): string {
        full_request_field_name = full_request_field_name || field_name;

        // if (context_query.anonimized_fields && context_query.anonimized_fields[api_type_id] && context_query.anonimized_fields[api_type_id][field_name]) {
        //     return "substring(encode(digest(" + full_request_field_name + " || 'SAC', 'sha256'), 'hex'), 1, 10)" + (do_not_happen_as ? "" : " AS " + field_name); // Le secret n'a aucune importance
        // } else {
        return full_request_field_name;
        // }
    }
    ...
    public static async check_is_anonymise<T extends IDistantVOBase>(datatable: ModuleTableVO, vos: T[], uid: number, user_data: IUserData): Promise<T[]> {
        ...
        for (const i in vos) {
            ...
            if (ServerAnonymizationController.registered_anonymization_field_conf_by_uid_vo_type_and_field_id[uid] &&
                ServerAnonymizationController.registered_anonymization_field_conf_by_uid_vo_type_and_field_id[uid][vo._type] &&
                ServerAnonymizationController.registered_anonymization_field_conf_by_uid_vo_type_and_field_id[uid][vo._type][vo.id]) {
    ...
    /**
     * TODO FIXME : à revoir, il faut une anonymisation réversible du coup...
     * @param row_field_value
     * @param row_field_api_type_id
     * @param row_field_field_id
     * @param uid
     */
    public static async get_unanonymised_row_field_value<T>(row_field_value: string, row_field_api_type_id: string, row_field_field_id: string, uid: number): Promise<string> {
    ...
    public static async broadcast_reload_conf() {
        await ForkMessageController.broadcast(new ServerAnonymizationReloadConfMessage());
    }
}